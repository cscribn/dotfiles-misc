...
If ($IsWindows -And (Get-Command mise -ErrorAction SilentlyContinue)) { $Env:Path += ";$env:LOCALAPPDATA\mise\shims" }
...
Import-Module PSReadLine; Set-PSReadLineOption -EditMode Windows -PredictionSource HistoryAndPlugin
